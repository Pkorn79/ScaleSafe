        ...
        <div class="flex-between mb-4">
          <div>
            ...
            <p class="text-sm text-muted">
              ScaleSafe automatically records supported GHL activity as client-level evidence.
            </p>
          </div>
          ...
          <div v-for="item in provisioningHealth.items" :key="item.key" class="health-row">
            ...
            <div class="health-row-content">
              ...
              <div v-if="item.key === 'pulse_diagnostics'" class="job-diagnostics-grid">
                ...
                <div class="text-sm text-muted">
                  Last pulse event delivered to GHL: {{ formatHealthTime(item.details?.recentPulseEventDeliveredAt || item.details?.recentPulseSentAt) }}
                  - Last outbound observed: {{ formatHealthTime(item.details?.recentPulseOutboundObservedAt) }}
                  - Last client submission: {{ formatHealthTime(item.details?.recentPulseSubmittedAt) }}
                </div>
                <div v-if="item.details?.formUrlSources" class="text-sm text-muted">
                  Pulse links are generated by ScaleSafe automatically.
                </div>
              </div>

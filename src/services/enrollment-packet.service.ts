...
function buildEnrollmentPacketHtml(data: PacketData): string {
  ...
  if (offer) {
    for (let i = 1; i <= 8; i++) {
      ...
      if (name) milestones.push(`${i}. ${name}${(offer as any)[`m${i}_delivers`] ? ' — ' + (offer as any)[`m${i}_delivers`] : ''}`);
    }
  }
...
<div style="margin-top:8px">
  <strong style="font-size:12px">Program Milestones:</strong>
  <div style="font-size:12px;margin-top:4px">${milestones.map(m => `<div style="padding:2px 0">${esc(m)}</div>`).join('')}</div>
</div>` : ''}
...
${clauseItems.length > 0 && offer?.compiled_tc_html ? `
<div style="margin-top:10px">
  <strong style="font-size:12px">Clause Acceptance:</strong>
...
<div class="footer">
  This document was automatically generated by ScaleSafe and represents the enrollment record as captured at the time of agreement. — ScaleSafe.io
</div>

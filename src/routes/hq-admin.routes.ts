...
router.post('/internal/hq/api/merchants/:locationId/test-access-approval', debugLimiter, requireHqToken, async (req: Request, res: Response) => {
  try {
    const locationId = req.params.locationId;
    const approved = req.body?.approved;
    if (typeof approved !== 'boolean') {
    ...
    const entitlement = await setTestAccessApproval({
      locationId,
      approved,
      approvedBy: adminLabel(req),
      note,
    });
    await audit(req, approved ? 'hq.approve_test_access' : 'hq.revoke_test_access', {
      targetLocationId: locationId,
      targetType: 'merchant',
      targetId: locationId,
      metadata: {
    ...
    logger.error({ err: err?.message || String(err), locationId: req.params.locationId }, 'HQ test access update failed');
    res.status(err?.statusCode || 500).json({ error: err?.message || 'Test access update failed' });
  }
});
...
function entitlementPanel(merchant){const e=merchant.entitlement||{};const wholepay=e.planKey==='wholepay';const test=e.planKey==='test';const controls=wholepay?'<div class="actions"><button onclick="setWholepayApproval(true)">Approve WholePay merchant</button><button class="danger" onclick="setWholepayApproval(false)">Revoke approval</button></div>':test?'<div class="actions"><button onclick="setTestAccessApproval(true)">Approve test access</button><button class="danger" onclick="setTestAccessApproval(false)">Revoke test access</button></div>':'';return '<div class="panel"><h3>Marketplace entitlement</h3><p><strong>'+escapeHtml(e.planLabel||'Unknown')+'</strong> <span class="pill '+(e.accessAllowed?'':'bad')+'">'+escapeHtml(e.accessState||'unknown')+'</span></p><p class="muted">'+escapeHtml(e.message||'')+'</p>'+controls+'</div>'}
...
async function setTestAccessApproval(approved){clearError();let note='';if(approved){note=window.prompt('Optional approval note (for example, beta user or promised free seat).')||''}else if(!window.confirm('Revoke free test access for this exact sub-account?')){return}try{await request('/internal/hq/api/merchants/'+encodeURIComponent(activeDetailLocation)+'/test-access-approval','POST',{approved,note});showNotice(approved?'Test access approved.':'Test access revoked.');await detail(activeDetailLocation);await load()}catch(e){showError(e.message)}}

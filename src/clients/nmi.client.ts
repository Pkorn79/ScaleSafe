...
export class NmiClient implements ProcessorInterface {
  ...
  async cancelSubscription(subscriptionId: string): Promise<{
    ...
    const responseText = String(nmi.responsetext || '');
    const notFound = !success && /(?:no recurring subscriptions? found|recurring subscriptions? (?:not found|does not exist))/i.test(responseText);

    return {
      success,
      errorMessage: !success ? responseText : undefined,

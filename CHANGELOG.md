...
### Security
- **External webhooks now share merchant-secret observe-mode validation and stable replay keys.** `/webhooks/external` now uses the same `requireMerchantWebhookSecret` middleware as GHL workflow webhooks, so Make.com-to-ScaleSafe posts can be logged/validated in observe mode before enforcement. External webhook idempotency no longer uses `Date.now()`; it now hashes the stable event identity/payload so identical replays produce the same event ID and can be deduplicated.

...
### Fixed
- **Dashboard value saved now uses the real defense outcome column.** `defense_outcomes` stores recovered value as `amount_recovered`, but dashboard reads and the defense outcome repository still referenced `amount_saved`. Updated dashboard overview/history totals and outcome inserts to use `amount_recovered`, and included the required `location_id` when recording defense outcomes.

...
describe('stripeCancelAtSeconds (#16 calendar cancel_at)', () => {
  it('computes cancel_at via calendar months, not fixed 30-day seconds', () => {
    ...
    // +2 calendar months = 2026-03-15, 1h before the (N+1)th anchor.
    const expected = Math.floor(Date.UTC(2026, 2, 15) / 1000) - 3600;
  ...
  it('handles annual via calendar years', () => {
    ...
    expect(stripeCancelAtSeconds(start, 'annual', 1)).toBe(Math.floor(Date.UTC(2027, 5, 10) / 1000) - 3600);
  });

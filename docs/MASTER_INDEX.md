...
| Command Center, Guardian, and reseller account dashboard | **Phase 1 implemented locally; staging certification pending** | [Command Center Architecture](COMMAND_CENTER_GUARDIAN_ARCHITECTURE_PLAN.md), [Phase 1 Certification](COMMAND_CENTER_PHASE_1_IMPLEMENTATION_AND_CERTIFICATION.md), [Verified Fable Review](COMMAND_CENTER_FABLE_ARCHITECTURE_REVIEW.md) | Operator identity/isolation remains default-off on `codex/beta-remediation`; no production SQL, deploy, DNS, or flag change has occurred. |

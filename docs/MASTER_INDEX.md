...
| Production schema | **Verified at 106; direct catalog gate pending** | Railway and the read-only production RPC identify exact project `zddyagfotdtfbcdursqu` at schema 106. API metadata shows no partial Command Center or migration 112 rollout. One forced-read-only PostgreSQL catalog check remains before migration approval. |

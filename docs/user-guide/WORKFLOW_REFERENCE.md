...
| Quick checkout | Client uses a quick checkout link | Client and payment are linked to the selected offer | Processor records the payment | Payment and enrollment evidence are created | Verified live for dual-option PIF: one client-linked PIF enrollment, no recurring state |

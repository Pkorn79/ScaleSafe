...
4. Add the documented operator variables and a new 32-byte operator encryption key. The operator key must differ from the processor encryption key. Use the publishable/anon Auth key from the same confirmed ScaleSafe project. Verify the actual Railway/Cloudflare proxy chain before setting `OPERATOR_TRUST_PROXY_HOPS`: it changes global Express client-IP handling, not only operator routes. Check both origin and proxied access; do not guess a hop count.

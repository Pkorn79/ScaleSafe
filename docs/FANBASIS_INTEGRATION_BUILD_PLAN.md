...
Phase 2 (optional, for seamless iframe UX on funnel Page 4): embedded checkout requires a FanBasis `product_id`. The public docs show `POST /public-api/checkout-sessions` creates the checkout product/session from inline ScaleSafe offer title + price and returns both `data.id` (short alphanumeric ID, e.g. `NLxj6`) and `payment_link`. Embedded checkout then accepts `product_id` with the same short-ID shape. Claude Code must confirm in sandbox that the session-response `data.id` can be passed directly as embedded `product_id`.

**2026-06-18 updated caveat — product ID handoff must be proven in sandbox.**
Public FanBasis docs show product/session creation happens through `POST /public-api/checkout-sessions`, not a standalone products CRUD flow. That response returns `data.id`, `checkout_session_id`, and `payment_link`; embedded checkout requires `creator_id` + `product_id`. The expected beta path is: ScaleSafe creates/reuses the FanBasis checkout session from the ScaleSafe offer, stores returned `data.id` as the FanBasis product ID, then creates an embedded checkout session using that ID. Sandbox must confirm this exact handoff before implementation treats it as binding. If sandbox disproves the handoff, fallback is hosted payment-link checkout using the same `payment_link`; do not make merchants manually paste product IDs.
...
2. **Phase F2 — Checkout path:** expected path is embedded checkout. First create/reuse a FanBasis checkout session from the ScaleSafe offer (`POST /public-api/checkout-sessions`) and store returned `data.id`, `checkout_session_id`, and `payment_link`; then create embedded session (`checkout_session_secret`) using `data.id` as `product_id`; render iframe on funnel Page 4 + Quick Checkout (client stays in funnel, consent forensics complete before payment); webhook receiver with signature validation; enrollment completion + evidence + triggers. **Riskiest assumption: confirm in sandbox that `data.id` from checkout-session creation is valid as embedded `product_id`. If not, fallback is hosted payment-link redirect from Page 4 using returned `payment_link`.**
...
3. **F4 embedded iframe — RESOLVED: preferred beta path, with hosted-link fallback.** Embedded checkout (iframe on funnel Page 4, client never leaves the consent-captured funnel) is the preferred beta experience. **Scope consequence:** embedded requires a FanBasis `product_id`. The docs indicate `POST /public-api/checkout-sessions` creates the product/session from inline offer fields and returns a short `data.id`; Claude Code must verify in sandbox that this ID feeds the embedded `product_id`. If it does, no merchant dashboard or manual product ID entry is needed. If sandbox disproves it, fall back to FanBasis hosted payment links created inline from the ScaleSafe offer. Hosted links redirect off the funnel on Page 4, but consent forensics remain valid because Pages 1-3 already captured and froze the packet before payment.
...
9. **Product ID path:** Embedded checkout requires `creator_id` + `product_id`. Public docs show checkout-session creation returns `data.id` and `payment_link`. Confirm in sandbox whether `data.id` is accepted as embedded `product_id`. If yes, plan the product/session sync service around that handoff. If no, plan hosted payment-link sessions created with inline ScaleSafe offer title + price. Do not make merchants manually paste product IDs as the fallback.
...
- 2026-06-18: Moved plan into repo `docs/` for Claude Code. Added product-ID caveat, then refined it after checking live docs: `POST /public-api/checkout-sessions` returns `data.id`, `checkout_session_id`, and `payment_link`; expected path is to store `data.id` and use it as embedded `product_id`, with sandbox confirmation before coding. Hosted `payment_link` remains fallback if sandbox disproves the handoff.

...
3. **F4 embedded iframe — RESOLVED: REQUIRED for beta.** Hosted payment-link redirect NOT acceptable for beta. Embedded checkout (iframe on funnel Page 4, client never leaves the consent-captured funnel) is in core beta scope. **Scope consequence:** embedded requires a FanBasis `product_id`. ScaleSafe should auto-create/sync a FanBasis product per offer only if FanBasis exposes a documented product create/update API. Claude Code must verify this during plan mode. If the API does not exist, the beta implementation must use a merchant-provided existing FanBasis `product_id` per offer, with validation/listing where possible. The embedded path also better preserves the consent-before-payment forensics sequence (no redirect away mid-funnel) — see Risk Register row on `payment_link` redirect, now largely mooted by this choice.
...
9. **Product ID path:** Embedded checkout requires `creator_id` + `product_id`. Does FanBasis expose a documented product create/update API that ScaleSafe can use for auto-sync? If yes, plan the product-sync service. If no, plan the beta fallback as merchant-pasted/selected existing FanBasis product IDs per offer. Do not build an assumed auto-sync path without a verified API contract.
...
- 2026-06-18: Moved plan into repo `docs/` for Claude Code. Added product-ID caveat: FanBasis embedded checkout requires `product_id`, but product auto-creation by API must be verified before implementation. If no create/update product API exists, beta uses merchant-provided FanBasis product IDs per offer.
